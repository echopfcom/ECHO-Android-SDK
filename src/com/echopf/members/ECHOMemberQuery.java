...
public class ECHOMemberQuery {
	...
	/**
	 * {@.en Finds members by synchronous communication.}
	 * {@.ja 同期通信によるメンバー検索。}
	 * 
	 * @param instanceId
	 ...
	 * @param params
	 * 		{@.en to control the output members by JSONObject}
	 ...
	 */
	public static ECHOArrayList<ECHOMemberObject> find(String instanceId, JSONObject params) throws ECHOException {
		return doFind(true, null, instanceId, params);
	}


	/**
	 * {@.en Finds members in a background thread.}
	 ...
	 * {@.ja 非同期通信によるログイン。ログイン後に指定したコールバックをメインスレッドで実行します。}
	 * 
	 * @param instanceId
	 * 		{@.en the reference ID of the finding target instance}
	 * 		{@.ja ログインメンバーが所属するインスタンスのID}
	 ...
	 * @param params to control the output members by JSONObject
	 * @throws ECHOException
	 */
	protected static ECHOArrayList<ECHOMemberObject> doFind(boolean sync, final FindCallback<ECHOMemberObject> callback, 
