...
/**
 * An ECHODataObject is an abstract data object in ECHO.
...
public abstract class ECHODataObject<S extends ECHODataObject<S>> extends ECHOObject {
	...
	/**
	 * Does Delete an object in a background thread.
	 * 
	 ...
	 * @throws ECHOException
	 */
	public ECHODate getDate(String name) throws ECHOException {
		try {
			return (ECHODate) data.get(name);
		} catch (JSONException e) {
			throw new ECHOException(e);
		}
	}
	...
	/**
	 * Returns the value mapped by name if it exists and is a ECHODate, or null otherwise.
	 */
	public ECHODate optDate(String name) {
		try {
			ECHODate date = this.getDate(name);
			return date;
		} catch(ECHOException e) {
			return null;
		}
	 ...
	 * Returns the value mapped by name if it exists, coercing it if necessary, or fallback if no such mapping exists.
	 */
	public ECHODate optDate(String name, ECHODate fallback) {
		try {
			ECHODate date = this.getDate(name);
			return date;
		} catch (ECHOException e) {
